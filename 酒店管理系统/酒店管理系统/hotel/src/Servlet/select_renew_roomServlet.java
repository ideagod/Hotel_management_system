...
public class select_renew_roomServlet extends HttpServlet {
	 public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException  
	    ...
	    public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException  {  
			...
			System.out.println("≤‚ ‘3"+customer_information);
			CustomerDaoImpl customerDao = new CustomerDao(); //Dao≤„
			pageResult<customer> cust3=customerDao.select_customer_bypage(page, customer_information);
			 session.setAttribute("cust3", cust3);	
				response.sendRedirect("tgls/agent/select_renew_room.jsp");
		}
		catch (Exception e) {
			e.printStackTrace();
