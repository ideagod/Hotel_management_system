		...
		<table class="layui-table">
			...
			<tbody>
				<c:forEach var="list3" items="${list3.list}">
					...
					<tr>
						<td>${list3.username}</td>
						<td>${list3.sex}</td>
						<td>${list3.phone}</td>
						<td>${list3.id_card}</td>
						<td>${list3.in_time}</td>
						<td>${list3.in_day}</td>
						<td>${list3.room_num}</td>
						<td>
							<form action="/hotel/renew1_roomServlet.do">
								<input type="hidden" name=room_num value="${list3.room_num}">
								<input type="hidden" name=in_day value="${ list3.in_day}">
								<p>
									续房天数：<select name="renew_day">
								...
								<button class="layui-btn layui-btn-xs"
									onclick="if(confirm('确定续房？')==false) return false;">续房</button>
							</form>
						</td>
					</tr>
	...
	background-color: #ddd;
		...
		<ul class="pagination">
			...
			<c:choose>
				...
				<c:otherwise>
					<li><a
						href="${pageContext.request.contextPath}/renew_roomServlet.do?&page=${list3.currentPage-1}">上一页</a></li>
				</c:otherwise>
			</c:choose>
			...
			<c:choose>
				...
				<c:otherwise>
					<li><a
						href="${pageContext.request.contextPath}/renew_roomServlet.do?&page=${list3.currentPage+1}">下一页</a></li>
				</c:otherwise>
			</c:choose>
